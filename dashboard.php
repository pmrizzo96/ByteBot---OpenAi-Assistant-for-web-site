...
if (!isset($_SESSION["login_effettuato"]) || $_SESSION["login_effettuato"] !== true) {
    ...
    header("Location: index.php"); // Reindirizza alla pagina di login se l'utente non è autenticato
    exit;
}
...
$query = "SELECT * FROM orders, users where (users.username='".$username."') and (orders.client_id=users.idutente)";
$result = $conn->query($query);
$ordini = array(); // Array per memorizzare gli ordini

if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $ordini[] = $row; // Aggiunge ogni articolo all'array
    }
} else {
...
<html lang="en">        
  ...
  <body>                   
    ...
    <p>Questa è la tua dashboard. Solo gli utenti autenticati possono accedere a questa pagina.</p>
	<br>
		    <div>
	<button onclick="window.location.href = 'carrello.php';">Vai al carrello</button>
	</div>
	<br>
	<p><h1>I tuoi ordini</h1></p>
    <div><table class="tg">
        <tr>
            <th>N. ordine</th>
            <th>Data</th>
        ...
        <?php foreach ($ordini as $ordine) { ?>
            <tr>
                <td><?php echo $ordine["order_id"]; ?></td>
                <td><?php echo $ordine["Order_date"]; ?></td>
                <td><?php echo $ordine["status"]; ?></td>
				<td><?php echo $ordine["totale"]; ?></td>
                <td>
                    <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
                        <input type="hidden" name="order_id" value="<?php echo $ordine["fattura"]; ?>">
                        <input type="submit" name="scarica_fattura" value="Fattura">
                    </form>
                </td>
            </tr>
        <?php } ?>
